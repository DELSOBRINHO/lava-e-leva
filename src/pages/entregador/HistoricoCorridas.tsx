...
export default function HistoricoCorridas() {
  ...
  const totalGanhos = orders.reduce((sum, o) => sum + (o.total_price || 0), 0);
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold mb-4 text-brand-dark">Histórico de Corridas</h1>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-white border border-brand-primary rounded-md p-4 shadow flex flex-col items-center">
          ...
          <div className="text-brand-dark">Entregas</div>
        </div>
        <div className="bg-white border border-brand-primary rounded-md p-4 shadow flex flex-col items-center">
          <div className="text-3xl font-bold text-brand-primary">R$ {totalGanhos.toFixed(2)}</div>
          <div className="text-brand-dark">Ganhos</div>
        </div>
        <div className="bg-white border border-brand-primary rounded-md p-4 shadow flex flex-col items-center">
      ...
      <div className="space-y-4">
        {orders.map((order) => (
          <div key={order.id} className="border border-brand-primary rounded-md p-4 shadow bg-white">
            ...
            <div className="text-sm">Entregue em: {order.updated_at ? new Date(order.updated_at).toLocaleString() : '-'}</div>
        ...
        {!loading && orders.length === 0 && <div className="text-brand-gray">Nenhuma entrega encontrada.</div>}
      </div>
    </div>
  );
} 